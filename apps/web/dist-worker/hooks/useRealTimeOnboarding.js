...
export function useRealTimeOnboarding(options = {}) {
    const { refreshInterval = 30000, enableNotifications = true, statusFilter, searchTerm } = options;
    ...
    const detectChanges = useCallback((oldCandidates, newCandidates) => {
        const updates = [];
            ...
            if (!oldCandidate) {
                ...
                if (oldCandidate.documents && newCandidate.documents) {
                    ...
                    if (newDocs.length > oldDocs.length) {
                        const newDocTypes = newDocs.filter(([type]) => !oldDocs.some(([oldType]) => oldType === type)).map(([type]) => type);
                        updates.push({
    ...
    const updateCandidate = useCallback(async (candidateId, updates) => {
        try {
            ...
            setCandidates(prev => prev.map(candidate => candidate.id === candidateId ? { ...candidate, ...updates } : candidate));
    ...
    if (!mounted) {
        return {
            ...
            addCandidate: async () => Promise.resolve({}),
            refresh: () => { },
            isRealTime: false
        };
    }
...
//# sourceMappingURL=useRealTimeOnboarding.js.map