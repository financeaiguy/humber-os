...
export function ChatWidget({ isOpen, onToggle, className = '' }: ChatWidgetProps) {
  ...
  return (
    <motion.div
      ...
      className={`fixed bottom-6 right-6 bg-slate-800/95 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-2xl z-50 flex flex-col ${className}`}
      style={{
        width: isMinimized ? 'auto' : '800px',
        height: isMinimized ? 'auto' : 'calc(85vh)',
        maxHeight: isMinimized ? 'auto' : '900px'
      }}
    >
      {/* Enhanced Header */}
          ...
          {isMinimized ? (
            ...
            <>
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-white">Humber AI Assistant</h3>
