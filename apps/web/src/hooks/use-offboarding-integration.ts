...
export function useOffboardingIntegration() {
  ...
  const fetchIntegrationData = async () => {
    try {
      ...
      // SECURITY: console statement removed: console.error('Error fetching integration data:', error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const updateBullPenStatus = async (engineerId: string, newStatus: 'available' | 'transitioning' | 'offboarded') => {
    ...
    // SECURITY: console statement removed: console.log(`Updating bull pen status for ${engineerId} to ${newStatus}`)
  ...
  const notifyStakeholders = async (offboardingRequest: any) => {
    ...
    // SECURITY: console statement removed: console.log('Sending notifications:', notifications)
